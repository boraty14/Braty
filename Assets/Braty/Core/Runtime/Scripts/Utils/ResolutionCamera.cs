...
namespace Braty.Core.Runtime.Scripts.Utils
{
    ...
    public class ResolutionCamera : MonoBehaviour
    {
        ...
        private Camera _resolutionCamera;
        private Vector2Int _screenSize;

        private void Awake()
        {
            _resolutionCamera = GetComponent<Camera>();
        }
        ...
        private void SetRatio()
        {
            ...
            if (currentAspect > targetAspect)
            {
                ...
                _resolutionCamera.rect = new Rect(start, 0.0f, width, 1.0f);
            }
            else if (currentAspect < targetAspect)
            {
                ...
                _resolutionCamera.rect = new Rect(0.0f, start, 1.0f, height);
            }
            else
            {
                // The screen is already the target aspect ratio,
                // so we set the camera rect to the full screen
                _resolutionCamera.rect = new Rect(0.0f, 0.0f, 1.0f, 1.0f);
            }
        }
    }
}